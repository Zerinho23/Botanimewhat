...
const METRICS = [
  ...
  { icon: MessageSquare, label: 'Grupos',    key: 'groups',        color: '#3B82F6', bg: 'rgba(59,130,246,.10)'  },
      ...
      {data.map((d, i) => {
        ...
        return (
          <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', height: '100%', justifyContent: 'flex-end', gap: 6, cursor: 'pointer' }}
            ...
            onMouseEnter={e => { const b = e.currentTarget.querySelector('.bar') as HTMLDivElement; if (b) { b.style.background = 'rgba(245,158,11,.75)'; b.style.boxShadow = '0 0 10px rgba(245,158,11,.30)' } }}
    ...
    <div style={{ ...card, padding: '16px 18px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
        ...
        <div style={{ display: 'flex', gap: 16 }}>
          ...
          <div style={{ textAlign: 'right' }}>
            <div style={{ fontSize: 16, fontWeight: 700, color: '#10B981', letterSpacing: '-0.02em' }}>{activeDays}</div>
      ...
      <div style={{ display: 'flex', gap: 6, overflowX: 'auto', paddingBottom: 4 }}>
        ...
        {/* Heatmap grid (column-first: each column = 1 week) */}
        <div className="heatmap-grid" style={{ flexShrink: 0 }}>
          {cells.map((c, i) => {
            ...
            const border = c.count === 0
              ? '1px solid rgba(255,255,255,0.05)'
              : `1px solid rgba(245,158,11,${Math.min(intensity * 0.7, 0.6)})`
            return (
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginTop: 10, justifyContent: 'flex-end' }}>
        ...
        {[0, 0.18, 0.4, 0.65, 1].map((op, i) => (
          <div key={i} style={{
            ...
            background: op === 0 ? 'rgba(255,255,255,0.04)' : `rgba(245,158,11,${op})`,
    ...
    <div style={{ marginBottom: 16 }}>
      ...
      <div className="stat-bar">
        <motion.div initial={{ width: 0 }} animate={{ width: pct + '%' }}
          ...
          className="stat-fill" style={{ background: color }}
        />
      </div>
    </div>
  )
  ...
  if (!isConfigured()) return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: 320 }}>
      <motion.div initial={{ opacity: 0, scale: 0.94 }} animate={{ opacity: 1, scale: 1 }}
        ...
        <p style={{ fontSize: 14, fontWeight: 600, marginBottom: 8 }}>Sin configurar</p>
        <p style={{ fontSize: 13, color: '#A1A1AA', lineHeight: 1.6 }}>
          Agrega <code style={{ background: 'rgba(245,158,11,.12)', border: '1px solid rgba(245,158,11,.25)', borderRadius: 4, padding: '1px 6px', color: '#FBBF24', fontSize: 12 }}>VITE_API_URL</code> en Vercel → Settings → Environment Variables.
        </p>
      </motion.div>
    </div>
  )
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }}
        style={{ ...card, padding: '13px 18px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 10, borderLeft: `3px solid ${connected ? '#10B981' : '#EF4444'}`, background: connected ? 'rgba(16,185,129,.04)' : 'rgba(239,68,68,.04)' }}>
        <div>
        ...
        <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.38, duration: 0.38 }}
          ...
          <div style={{ flex: 1, overflowY: 'auto', maxHeight: 200 }}>
            <AnimatePresence initial={false}>
              {recentEvs.length === 0
                ? <div className="empty-state"><div className="empty-state-icon"><Activity size={16} color="#52525B" /></div><div className="empty-state-title">Sin eventos</div></div>
                : recentEvs.map((ev, i) => {
      ...
      <div className="grid-trio">
        ...
        <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.46, duration: 0.38 }}
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '14px 18px', borderBottom: bdr }}>
            <Award size={14} color="#71717A" /><span style={{ fontSize: 13, fontWeight: 600 }}>Ranking XP</span>
          </div>
          <div style={{ flex: 1 }}>
            {topUsers.length === 0
              ? <div className="empty-state"><div className="empty-state-icon"><Users size={16} color="#52525B" /></div><div className="empty-state-title">Sin datos</div></div>
              : topUsers.map((u, i) => (
        ...
        <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.51, duration: 0.38 }}
          style={{ ...card, display: 'flex', flexDirection: 'column' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '14px 18px', borderBottom: bdr }}>
            <Zap size={14} color="#71717A" /><span style={{ fontSize: 13, fontWeight: 600 }}>Ranking Cmds</span>
          </div>
          <div style={{ flex: 1 }}>
            {topCmds.length === 0
              ? <div className="empty-state"><div className="empty-state-icon"><Zap size={16} color="#52525B" /></div><div className="empty-state-title">Sin datos</div></div>
              : topCmds.map((u, i) => (
                <div key={u.jid} className="lb-row">
                  <span className={`lb-pos lb-pos-${i < 3 ? i + 1 : 'n'}`}>{i + 1}</span>
                  <div style={{ width: 26, height: 26, borderRadius: '50%', background: 'rgba(139,92,246,.10)', border: '1px solid rgba(139,92,246,.22)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, fontWeight: 700, color: '#A78BFA', flexShrink: 0 }}>
                    {(u.name || u.jid || '?').slice(0, 2).toUpperCase()}
                  </div>
                  <span className="lb-name">{u.name || u.jid || 'Usuario'}</span>
        ...
        <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.56, duration: 0.38 }}
          style={{ ...card, display: 'flex', flexDirection: 'column' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '14px 18px', borderBottom: bdr }}>
            <Shield size={14} color="#71717A" /><span style={{ fontSize: 13, fontWeight: 600 }}>Sistema</span>
          </div>
          <div style={{ padding: '16px 18px 12px' }}>
            {stats ? (
              ...
              <div className="empty-state"><div className="empty-state-icon"><Shield size={16} color="#52525B" /></div><div className="empty-state-title">Sin datos</div></div>
            )}
          </div>
        </motion.div>
      </div>
